# Viam Docs: Information Architecture

This is the living navigation for the new Viam documentation structure. Click any link to view the current state of that content. Status indicators show what exists.

**Status Key:**
- 🔴 Placeholder — structure only, no content
- 🟡 Draft — working content, incomplete
- 🟢 Complete — ready for review

---

## [Understand](./understand/INDEX.md) 🟡
*What is Viam and why should I care?*

- [What is Viam](./understand/what-is-viam.md) 🔴
- [How Viam Works](./understand/how-viam-works.md) 🔴
- [What Problems Viam Solves](./understand/problems-viam-solves.md) 🟡

---

## [Try](./try/INDEX.md) 🔴
*Zero-friction simulation experiences — full journey through all 5 stages*

- [Stationary Vision Simulation](./try/stationary-vision.md) 🔴
- [Mobile Base Simulation](./try/mobile-base.md) 🔴
- [Arm + Vision Simulation](./try/arm-vision.md) 🔴

---

## [Build](./build/INDEX.md) 🟡
*Task-oriented blocks — modular tutorials you compose*

### [Foundation](./build/foundation/INDEX.md) 🔴
- [Connect to Cloud](./build/foundation/connect-to-cloud.md) 🔴
- [Add a Camera](./build/foundation/add-camera.md) 🔴
- [Capture and Sync Data](./build/foundation/capture-sync-data.md) 🔴
- [Run ML Inference](./build/foundation/run-ml-inference.md) 🔴

### [Perception](./build/perception/INDEX.md) 🔴
- [Detect Objects (2D)](./build/perception/detect-objects-2d.md) 🔴
- [Classify Objects](./build/perception/classify-objects.md) 🔴
- [Track Objects Across Frames](./build/perception/track-objects.md) 🔴
- [Measure Depth](./build/perception/measure-depth.md) 🔴
- [Localize Objects in 3D](./build/perception/localize-3d.md) 🔴

### [Stationary Vision](./build/stationary-vision/INDEX.md) 🔴
- [Trigger on Detection](./build/stationary-vision/trigger-on-detection.md) 🔴
- [Count Objects](./build/stationary-vision/count-objects.md) 🔴
- [Inspect for Defects](./build/stationary-vision/inspect-defects.md) 🔴
- [Monitor Over Time](./build/stationary-vision/monitor-over-time.md) 🔴

### [Mobile Base](./build/mobile-base/INDEX.md) 🔴
- [Control Motors](./build/mobile-base/control-motors.md) 🔴
- [Read Odometry](./build/mobile-base/read-odometry.md) 🔴
- [Build a Map](./build/mobile-base/build-map.md) 🔴
- [Navigate to Waypoint](./build/mobile-base/navigate-waypoint.md) 🔴
- [Avoid Obstacles](./build/mobile-base/avoid-obstacles.md) 🔴
- [Follow a Patrol Route](./build/mobile-base/patrol-route.md) 🔴

### [Arm + Manipulation](./build/arm-manipulation/INDEX.md) 🔴
- [Move Joints](./build/arm-manipulation/move-joints.md) 🔴
- [Control Gripper](./build/arm-manipulation/control-gripper.md) 🔴
- [Move to Pose](./build/arm-manipulation/move-to-pose.md) 🔴
- [Pick an Object](./build/arm-manipulation/pick-object.md) 🔴
- [Place an Object](./build/arm-manipulation/place-object.md) 🔴
- [Visual Servoing](./build/arm-manipulation/visual-servoing.md) 🔴

### [Integration](./build/integration/INDEX.md) 🔴
- [Detect While Moving](./build/integration/detect-while-moving.md) 🔴
- [Navigate to Detected Object](./build/integration/navigate-to-detected.md) 🔴
- [Pick from Bin](./build/integration/pick-from-bin.md) 🔴
- [Mobile Pick-and-Place](./build/integration/mobile-pick-place.md) 🔴
- [Multi-step Task Sequence](./build/integration/multi-step-task.md) 🔴

---

## [Deploy](./deploy/INDEX.md) 🔴
*Stage 2: First Deployment problems*

- [Remote Access and NAT Traversal](./deploy/remote-access.md) 🔴
- [Configuration in the Field](./deploy/field-configuration.md) 🔴
- [Handling Connectivity Issues](./deploy/connectivity.md) 🔴
- [Alerts and Monitoring](./deploy/alerts-monitoring.md) 🔴
- [Environment Differences](./deploy/environment-differences.md) 🔴

---

## [Scale](./scale/INDEX.md) 🔴
*Stages 3-4: Multiple Units and Fleet at Scale problems*

- [Provisioning New Devices](./scale/provisioning.md) 🔴
- [Configuration Management](./scale/configuration-management.md) 🔴
- [Handling Hardware Variance](./scale/hardware-variance.md) 🔴
- [Fleet Monitoring](./scale/fleet-monitoring.md) 🔴
- [OTA Updates](./scale/ota-updates.md) 🔴
- [Staged Rollouts](./scale/staged-rollouts.md) 🔴
- [Multi-tenancy](./scale/multi-tenancy.md) 🔴

---

## [Maintain](./maintain/INDEX.md) 🔴
*Stage 5: Ongoing Maintenance problems*

- [Remote Debugging](./maintain/remote-debugging.md) 🔴
- [Log Access](./maintain/log-access.md) 🔴
- [Pushing Hotfixes](./maintain/hotfixes.md) 🔴
- [Model Updates and Retraining](./maintain/model-updates.md) 🔴
- [Performance Degradation](./maintain/performance-degradation.md) 🔴

---

## [Reference](./reference/INDEX.md) 🔴
*API and component documentation*

- [Components](./reference/components.md) 🔴
- [Services](./reference/services.md) 🔴
- [SDKs](./reference/sdks.md) 🔴
- [API](./reference/api.md) 🔴

---

## [Work Cell Guides](./work-cells/INDEX.md) 🔴
*Curated paths through blocks for specific use cases*

- [Warehouse AMR](./work-cells/warehouse-amr.md) 🔴
- [Quality Inspection](./work-cells/quality-inspection.md) 🔴
- [Pick and Place](./work-cells/pick-and-place.md) 🔴
- [Agricultural Monitoring](./work-cells/agricultural-monitoring.md) 🔴

---

## Planning Documents

These are our working documents, not user-facing content:

- [Lifecycle Problems Analysis](./planning/lifecycle-problems.md) 🟢
- [Target User Definition](./planning/target-user.md) 🟢
- [Onboarding Strategy](./planning/onboarding-strategy.md) 🟡
- [Block Definitions](./planning/block-definitions.md) 🟡
- [Hardware Recommendations](./planning/hardware-recommendations.md) 🔴
- [Simulation Requirements](./planning/simulation-requirements.md) 🔴

---

## Quick Stats

| Section | Total Items | 🔴 | 🟡 | 🟢 |
|---------|-------------|-----|-----|-----|
| Understand | 3 | 2 | 1 | 0 |
| Try | 3 | 3 | 0 | 0 |
| Build | 25 | 25 | 0 | 0 |
| Deploy | 5 | 5 | 0 | 0 |
| Scale | 7 | 7 | 0 | 0 |
| Maintain | 5 | 5 | 0 | 0 |
| Reference | 4 | 4 | 0 | 0 |
| Work Cells | 4 | 4 | 0 | 0 |
| Planning | 6 | 2 | 2 | 2 |
| **Total** | **62** | **57** | **3** | **2** |
